    ...
    Column {
      ...
      Column {
        ...
        Text {
          width: parent.width
          wrapMode: Text.Wrap
          text: "• Setup: Select WhatsApp in OmaChat, choose Use a QR code, open WhatsApp on your phone > Linked devices > Link a device, and scan the displayed QR code. Keep your phone online during linking and initial sync.\n• Features: Real-time conversation sync with saved contact names, text, photos, captions, reactions, cross-platform M4A audio clips (with ffmpeg/ffplay), local GIF files and optional GIPHY search (with ffmpeg), retryable media downloads, and incoming static WebP stickers.\n• Limitations: History reflects initial phone sync and live messages; additional history cannot be requested from the phone, but cached history can be paged. Ephemeral and view-once media are intentionally not saved or reopened. WhatsApp audio uses standard clips rather than native PTT so it remains playable on iPhone and Android. Calling is currently unavailable."
          color: root.mutedColor
          font.family: root.fontFamily
          font.pixelSize: fs(Style.font.body)
          lineHeight: 1.25
