...
- Telegram support in this milestone integrates the pure Go gotd MTProto client behind a testable abstraction and context-safe QR login flow (`StartPairing`), restoring persisted sessions via `~/.local/share/omachat/telegram.session` with strict credential validation and state machine events. Telegram UI remains Coming Soon pending dialog sync, messaging RPCs, and live verification.
